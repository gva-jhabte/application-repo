...
def build_flow(context: dict):
    ...
    extract_qids = ExtractQidSummaryOperator()
    ...
    flow = extract_qids > save_to_bucket > end
...
def main(context: dict = {}):
    ...
    for date in date_range(start_date, end_date):
        logger.debug(F'Starting QID_SUMMARY job for {date}')
        my_context = context.copy()
        my_context['date'] = date
        execute_day_of_processing(context=my_context)
        logger.debug(F'Completed QID_SUMMARY job for {date}')


if __name__ == "__main__":
    context = {}
    context['config_file'] = 'QID_SUMMARY.metadata'
    main(context)
